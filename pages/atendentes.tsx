...
export default function AtendentesPage() {
    const [atendentes, setAtendentes] = useState<AtendenteResponse[]>([])
    ...
    const atualizarDados = async () => {
        try {
            const todosAtendentes: AtendenteResponse[] = []
            for (const time of TIMES) {
                const response = await fetch(`/api/atendimento/atendentes/${time}`)
                const atendentesDoTime: AtendenteResponse[] = await response.json()
                        ...
                        {atendentes.map((atendente) => (
                            ...
                            >
                                ...
                                <div className="flex items-center">
                                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${atendente.atendimentosAtuais === 3 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                                        }`}>
